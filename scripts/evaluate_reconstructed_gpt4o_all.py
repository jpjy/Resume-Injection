"""Resume-safe full GPT-4o evaluation of all 200 x 3 PDF/job conditions.
...
from evaluate_gpt4o_pilot import MODEL, response_text


ROOT = Path(__file__).resolve().parents[1]
DATASET = ROOT / "reconstructed_dataset_200"
OUTPUT = DATASET / "gpt4o_full_200_pairs"
LEVELS = [
    (0, 30, "not recommend"),
...
def call_gpt4o(key: str, job_text: str, resume_text: str) -> dict:
    payload = {
        ...
        "text": {"format": {"type": "json_schema", "name": "resume_recommendation",
    ...
    for attempt in range(7):
        try:
            ...
            if isinstance(score, bool) or not isinstance(score, int) or not 0 <= score <= 100:
                raise RuntimeError(f"Invalid GPT-4o fit score: {score}")
            ...
            if attempt == 6:
                raise RuntimeError(f"GPT-4o output invalid after retries: {error}") from None
...
def run_task(key: str, task: dict) -> dict:
    result = call_gpt4o(key, task["job_text"], task["resume_text"])
...
def validate_existing(task: dict) -> bool:
    ...
    if result["model"] != MODEL or result["evaluation"]["recommendation_level"] != LEVEL_BY_SCORE[score]:
...
def export_results(tasks: list[dict]) -> None:
    ...
    (OUTPUT / "method.md").write_text(
        ...
        "`gpt-4o-2024-11-20` model, temperature 0, structured-output schema, and prompt "
        "are used for all 600 calls. The API key is read at runtime and never saved.\n\n"
        ...
        "is GPT-4o's response, normalized to a single line. `pass_level` in the CSV "
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if done % 25 == 0 or done == 600:
                        print(f"Completed {done}/600 GPT-4o evaluations", flush=True)
